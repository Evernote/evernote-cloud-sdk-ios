...
This document gives a few quick examples and some discussion of the primary classes.
...
ENSession.shared().uploadNote(note, notebook: nil) { (noteRef: ENNoteRef?, uploadNoteError: Error?) in
	// ...
}
```
...
```swift
ENSession.shared().downloadNote(noteRef, progress: nil) { (note: ENNote?, error: Error?) in
}
```
